...
/*    */ {
	...
	/*    */ {
		...
		/*    */ {
			...
			/*    */ {
				/* 23 */ String ping = null;
				/* 24 */ j = (arrayOfMethod = p.getClass().getMethods()).length;
				for (i = 0; i < j; i++) {
					...
					/* 25 */ if (meth.getName().equals("getHandle")) {
						...
						/*    */ {
							...
							/* 29 */ m = (arrayOfField = obj.getClass().getFields()).length;
							for (k = 0; k < m; k++) {
		...
		/*    */ {
			...
			/* 52 */ String ping = null;
			/* 53 */ j = (arrayOfMethod = p.getClass().getMethods()).length;
			for (i = 0; i < j; i++) {
				Method meth = arrayOfMethod[i];
				/* 54 */ if (meth.getName().equals("getHandle")) {
					...
					/*    */ {
						...
						/* 58 */ m = (arrayOfField = obj.getClass().getFields()).length;
						for (k = 0; k < m; k++) {
							Field field = arrayOfField[k];
 ...
 * plugins\JaapaPvP.jar!\com\Nerio\net\commands\Ping.class Java compiler
