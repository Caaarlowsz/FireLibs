...
/*     */ {
	...
	/*     */ {
		...
		/* 43 */ for (Entity entity : this.controller.world.getWorld().getEntities()) {
			...
			/* 46 */ (entity.getEntityId() != this.owner.getBukkitEntity()/* 47 */ .getEntityId())) {
				/* 48 */ if (entity.getEntityId() != this.controller/* 49 */ .getBukkitEntity().getEntityId()) {
					/* 50 */ if (entity.getLocation().distance(this.controller.getBukkitEntity().getLocation()) >= 2.0D)
					/*     */ {
						/* 52 */ if ((entity instanceof Player)) {
							/* 53 */ ((Player) entity).getEyeLocation()
	...
	/*     */ {
		/* 97 */ this.sb = ((Snowball) this.owner.getBukkitEntity().launchProjectile(/* 98 */ Snowball.class));
		...
		/*     */ {
			...
			/* 110 */ p.getWorld().playSound(p.getLocation(), org.bukkit.Sound.STEP_GRAVEL, 1.0F, /* 111 */ 1.0F);
			/*     */ }
 ...
 * plugins\MasterKits.jar!\Kits\Cordinha.class Java compiler version: 7 (51.0)
